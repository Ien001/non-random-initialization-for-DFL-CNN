from model.DFL import DFL_VGG16, DFL_ResNet, DFL_ResNet_for_sample
...
parser.add_argument('--save_epoch', default=2, type=int,
                    help='every save_epoch we will save')
parser.add_argument('--w', default=448, type=int,
                    help='transform, seen as align')
parser.add_argument('--h', default=448, type=int,
                    help='transform, seen as align')
parser.add_argument('--eval_epoch_thershold', default=60, type=int,
                    help='eval_epoch_thershold')
...
def main():
    ...
    # for DFL_ResNet model
    #model = DFL_ResNet(k = 10, nclass = 200)     
    model_for_sample = DFL_ResNet_for_sample(k = 10, nclass = 200)
    model_for_sample.cuda()
    ...
    if args.init_type is not None:
        # for conv6 inital 
        # non-random initiallization 
        sampledir = '/path/to/your/workplace/DFL-CNN/dataset/sample_from_train/'
        transform_test_simple = get_transform_for_test_simple()
        test_dataset_simple = ImageFolderWithPaths(sampledir, transform = transform_test_simple)
        test_loader_simple = torch.utils.data.DataLoader(
            test_dataset_simple, batch_size=1, shuffle=False,
            num_workers=args.workers, pin_memory=True, drop_last = False)

        center = validate_simple_ian(args, test_loader_simple, model_for_sample)
    ...
    sampledir = os.path.join(dataroot, 'sample_from_train')
    ...
    test_dataset  = ImageFolderWithPaths(testdir,  transform = transform_test_simple)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        print('init_start_epoch', args.start_epoch)
        adjust_learning_rate(args, optimizer, epoch, gamma = 0.1)

        train(args, train_loader, model, criterion, optimizer, epoch)

        # evaluate on validation set
        if epoch % args.eval_epoch == 0 and epoch != 0 and epoch > args.eval_epoch_thershold:
